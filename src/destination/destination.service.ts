...
export class DestinationService {
    ...
    async findDestinationById(touristId: string, tripId: number, desId: number): Promise<DesResponse> {
    ...
    async createDestination(touristId: string, tripId: number, req: CreateDesDto): Promise<DesResponse> {
        const checkTourist = await this.touristRepository.findById(touristId)
        if(!checkTourist) {
            throw new HttpException('tourist not found', 404)
        }
    ...
    async updateDestination(touristId: string, tripId: number, desId: number, req: UpdateDesDto): Promise<DesResponse> {
        const checkTourist = await this.touristRepository.findById(touristId)
        if(!checkTourist) {
            throw new HttpException('tourist not found', 404)
        }
    ...
    async deleteDestination(touristId: string, tripId: number, desId: number): Promise<boolean> {
        const checkTourist = await this.touristRepository.findById(touristId)
        if(!checkTourist) {
            throw new HttpException('tourist not found', 404)
        }
