...
export class TripService {
    ...
    async findAllTrips(touristId: string, page: number, perPage: number): Promise<TripResponse[]> {
    ...
    async createTrip(touristId: string, req: CreateTripDto): Promise<TripResponse> {
        const checkTourist = await this.touristRepository.findById(touristId)
        if(!checkTourist) {
            throw new HttpException('tourist not found', 404)
        }
    ...
    async updateTrip(touristId: string, tripId: number, req: UpdateTripDto): Promise<TripResponse> {
        const checkTourist = await this.touristRepository.findById(touristId)
        if(!checkTourist) {
            throw new HttpException('tourist not found', 404)
        }
