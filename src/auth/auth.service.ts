...
export class AuthService {
    ...
    async createAdmin(req: RegisterAdminDto): Promise<AdminResponse> {
        const checkEmailAdmin = await this.authRepository.checkEmailAdmin(req.email)
        if (checkEmailAdmin){
    ...
    async loginAdmin(req: LoginDto): Promise<AdminResponse> {
        ...
        const comparePassword = bcrypt.compareSync(req.password, checkAdmin.password)
        if(!comparePassword){
            throw new HttpException('email or password is invalid', 400)
        }
        return checkAdmin
    ...
    async loginTourist(req: LoginDto): Promise<TouristResponse> {
        ...
        const comparePassword = bcrypt.compareSync(req.password, checkTourist.password)
        if(!comparePassword){
            throw new HttpException('email or password is invalid', 400)
        }
        return checkTourist
