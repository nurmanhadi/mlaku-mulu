...
export class AuthController {
    ...
    @HttpCode(201)
    async registerAdmin(@Body() req: RegisterAdminDto): Promise<JsonResponse<AdminResponse>> {
    ...
    async refreshTokenTourist(@Req() req: Request) {
        const newAccessToken = await this.authService.generateNewAccessTokenTourist(req)
        return {
            statusCode: 200,
            message: 'generate new access token success',
            access_token: newAccessToken
